package com.github.liudi0105.summer.core.ioc.utils;
...
public class PackageScanner {
    ...
    private List<String> loadResource() throws IOException {
        ...
        while (urls.hasMoreElements()) {
            ...
            PathUtils.ResourceType type = determineType(u);
     ...
     * @return
     */
    private PathUtils.ResourceType determineType(URL url) {
        if (url.getProtocol().equals(PathUtils.ResourceType.FILE.getTypeString())) {
            return PathUtils.ResourceType.FILE;
        }

        if (url.getProtocol().equals(PathUtils.ResourceType.JAR.getTypeString())) {
            return PathUtils.ResourceType.JAR;
        }
    ...
    private List<String> scanJar(String path) throws IOException {
        ...
        while (entries.hasMoreElements()) {
            ...
            if ((name.startsWith(pkgPath)) && (name.endsWith(PathUtils.ResourceType.CLASS_FILE.getTypeString()))) {
    ...
    private List<String> scanFile(String path, String basePkg) {
        ...
        if (null != files) {
            ...
            for (int ix = 0; ix < LEN; ++ix) {
                ...
                } else if (file.getName().endsWith(PathUtils.ResourceType.CLASS_FILE.getTypeString())) {
                    // 如果是以.class结尾
